...
export interface ISubscriptionService {
  getSubscriptions(user: string): Promise<ISubscription[]>
  getSubscription(id: string, user: string): Promise<ISubscription | null>
  addSubscription(subscription: ISubscription): Promise<ISubscription>
  editSubscription(id: string, value: Partial<ISubscription>): Promise<ISubscription>
  deleteSubscription(id: string): Promise<void>
  getActiveSubscriptions(user: string): Promise<ISubscription[]>
  getTransactionsBySubscription(id: string, user: string): Promise<ITransaction[]>
  getMatchingTransactions(id: string, user: string): Promise<ITransaction[]>
...
export default class SubscriptionService implements ISubscriptionService {
  async getSubscriptions (user: string): Promise<ISubscription[]> {
  ...
  async getSubscription (id: string, user: string): Promise<ISubscription | null> {
  ...
  async editSubscription (id: string, value: Partial<ISubscription>): Promise<ISubscription> {
    return SubscriptionModel.findByIdAndUpdate(id, value, { new: true }) as unknown as ISubscription
  }
  ...
  async getActiveSubscriptions (user: string): Promise<ISubscription[]> {
    return SubscriptionModel.find({ user })
  }

  async getTransactionsBySubscription (id: string, user: string): Promise<ITransaction[]> {
  ...
  async getMatchingTransactions (id: string, user: string): Promise<ITransaction[]> {
    const subscription = await SubscriptionModel.findOne({ _id: id, user }) as unknown as ISubscription
  ...
  async recalculateNextPaymentDate (subscriptionId: string): Promise<void> {
    const subscription = await SubscriptionModel.findById(subscriptionId) as unknown as ISubscription
    if (!subscription) return

    const lastTx = await TransactionModel.findOne({ subscriptionId })
      .sort({ date: -1 }) as unknown as ITransaction | null
