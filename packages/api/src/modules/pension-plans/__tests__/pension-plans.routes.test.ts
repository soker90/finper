...
describe('Pension Plans Routes', () => {
  ...
  describe('CRUD /api/pension-plans', () => {
    ...
    test('GET / returns every plan of the user', async () => {
      await supertest(server.app).post(path).auth(token, { type: 'bearer' }).send({ name: 'Plan A' })
      await supertest(server.app).post(path).auth(token, { type: 'bearer' }).send({ name: 'Plan B' })
    ...
    test('PATCH /:id edits the plan name', async () => {
      const created = await supertest(server.app).post(path).auth(token, { type: 'bearer' }).send({ name: 'Old name' })
    ...
    test('DELETE /:id deletes the plan', async () => {
      const created = await supertest(server.app).post(path).auth(token, { type: 'bearer' }).send({ name: 'To delete' })
    ...
    test('a plan from another user is not reachable', async () => {
      ...
      const created = await supertest(server.app).post(path).auth(otherToken, { type: 'bearer' }).send({ name: 'Rogue plan' })
  ...
  describe('Movements', () => {
    ...
    test('POST /:id/movements with missing fields returns 422', async () => {
      const plan = await supertest(server.app).post(path).auth(token, { type: 'bearer' }).send({ name: 'Plan' })
    ...
    test('using a movement id under another plan returns 404', async () => {
      const planA = await supertest(server.app).post(path).auth(token, { type: 'bearer' }).send({ name: 'Plan A' })
      const planB = await supertest(server.app).post(path).auth(token, { type: 'bearer' }).send({ name: 'Plan B' })
