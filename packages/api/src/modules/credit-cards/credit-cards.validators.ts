...
export const validateCreditCardCreateParams = (body: Record<string, any>): Record<string, any> => {
  const { error, value } = createCardSchema.validate(body)
  if (error) throw Boom.badData(error.message).output
  return value
}

export const validateCreditCardExist = async (id: string, user: string): Promise<any> => {
...
export const validateCreditCardMovementCreateParams = (body: Record<string, any>): Record<string, any> => {
  const { error, value } = createMovementSchema.validate(body)
  if (error) throw Boom.badData(error.message).output
  return value
}

export const validateCreditCardMovementEditParams = (body: Record<string, any>): Record<string, any> => {
  const { error, value } = editMovementSchema.validate(body)
  if (error) throw Boom.badData(error.message).output
  return value
}

export const validateCreditCardPayDebtParams = (body: Record<string, any>): Record<string, any> => {
