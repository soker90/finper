...
- **`better-sqlite3` is Node-ABI-locked** (unlike `bcrypt`, which uses N-API and is version-stable). If you switch Node versions with `fnm`/`nvm` and then run the app without reinstalling, it crashes at startup with a native assertion (`RemoveEnvironmentCleanupHook` / `NODE_MODULE_VERSION` mismatch) instead of a clear JS error. Run `make fix-native` to re-fetch the correct prebuilt binary for the active Node version. Do **not** fix this with `node-gyp rebuild`, which can produce a binary that compiles fine but crashes at process teardown.
